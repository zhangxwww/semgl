...
class GraphLearner(nn.Module):
    ...
    def forward(self, context, ctx_mask=None):
        ...
        if self.metric_type == 'attention':
            ...
            expand_weight_tensor = self.weight_tensor.unsqueeze(1)  # num per * 1 * input size
            if len(context.shape) == 3:
                expand_weight_tensor = expand_weight_tensor.unsqueeze(1)  # num per * 1 * 1 * input size

            context_fc = context.unsqueeze(0) * expand_weight_tensor  # num per * batch size * ctx size * input size
            context_norm = F.normalize(context_fc, p=2, dim=-1)   # num per * batch size * ctx size * input size
            attention = torch.matmul(context_norm, context_norm.transpose(-1, -2)).mean(0)  # batch size * ctx size * ctx size
            markoff_value = 0
            ...
            if self.cached is None:
                ...
                a1 = c.unsqueeze(0)
                a2 = c.unsqueeze(1)
                attention = ((a1 - a2) ** 2).mean(2)
                attention = torch.exp(-attention).cuda()

                self.cached = attention

            attention = self.cached.detach()
            '''
            '''
            c = context.cpu()
            a1 = c.unsqueeze(0)
            ...
            attention = torch.tensor(pairwise_distances(context.cpu().detach().numpy())).cuda()
            attention = torch.exp(-attention / 2)
            d = attention.sum(0)
            d_sqrt_inv = torch.sqrt(1.0 / (d + eps))
            d1 = d_sqrt_inv.unsqueeze(1).repeat(1, n)
            d2 = d_sqrt_inv.unsqueeze(0).repeat(n, 1)
            attention = d1 * attention * d2
            markoff_value = 0
        ...
        #np.save('origin.npy', attention.detach().cpu().numpy())
        ...
        #np.save('after.npy', attention.detach().cpu().numpy())
