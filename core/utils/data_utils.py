...
from .cub_data import data_utils as cub_data_utils
from .visual_object_data import data_utils as visual_data_utils
from .sun_data import data_utils as sun_data_utils
from .timer import Timer
from . import padding_utils
from . import constants

def onehot(y):
    m = int((torch.max(y) + 1).item())
    n = y.shape[0]
    oh = torch.zeros((n, m)).to(y.device).scatter_(1, y.view(-1, 1), 1)
    return oh
...
def prepare_datasets(config):
    ...
    elif config['data_type'] == 'sun':

        features, labels, idx_train, idx_val, idx_test = sun_data_utils.load_data(config['data_dir'], config)

        device = config['device']
        data = {'features': features.to(device) if device else features,
                'labels': labels.to(device) if device else labels,
                'idx_train': idx_train.to(device) if device else idx_train,
                'idx_val': idx_val.to(device) if device else idx_val,
                'idx_test': idx_test.to(device) if device else idx_test}

    elif config['data_type'] == 'visual_object':

        features, labels, idx_train, idx_val, idx_test = visual_data_utils.load_data(config['data_dir'], config['feature_name'], config['dataset_name'])
    ...
    elif config['data_type'] == 'cub':

        features, labels, idx_train, idx_val, idx_test = cub_data_utils.load_data(config['data_dir'], no_val=config['no_val'])
