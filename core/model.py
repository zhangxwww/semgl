...
class Model(object):
    ...
    def __init__(self, config, train_set=None, args=None):
        self.config = config
        self.args = args
        ...
        if config['data_type'] == 'text':
            saved_vocab_file = os.path.join(config['data_dir'], '{}_seed{}.vocab'.format(config['dataset_name'],
                                                                                         config.get('data_seed', 1234)))
    ...
    def init_saved_network(self, saved_dir):
        ...
        if self.config['data_type'] == 'text':
            ...
            self.network = self.net_module(self.config, self.args)
